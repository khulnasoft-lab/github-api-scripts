...
def main(args):
    ...
    p = Path('test-data/workflow-simple-self-hosted.yml_')
    json_file = f"""tmp/workflow-simple-self-hosted.json"""
    filename_in_repo = f"""workflow-simple-self-hosted.yml"""
    with open(p, 'rb') as ct:
       t = {}
       chapter_content = ct.read()
       chapter_base64 = base64.encodebytes(chapter_content)
       t["message"] = f"""A simple workflow file for a self hosted runner with The Power."""
       t["committer"] = {}
